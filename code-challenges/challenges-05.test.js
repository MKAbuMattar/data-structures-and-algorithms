...
const toLastNames = people => {
  ...
  let newArr = [];
  people.map(result =>
    newArr.push(`${result.firstName} ${result.lastName}`)
  );
...
const reversedString = (str) => {
  ...
  let i = strLetters.length - 1; // i === index
  let reverseAtHome = [];
  strLetters.reduce((strTemp) => {
    strTemp.push(strLetters[i]);
    i--;
    return strTemp;
  }, reverseAtHome);

  return reverseAtHome.join('');
};
