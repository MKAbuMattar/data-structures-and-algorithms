...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((accumulator, sourceArr) => sourceArr.filter(result => result === target).length + accumulator, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  let sum = 0;
  input.map(result => result.map(result => sum += result));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(resultArr => resultArr.filter(result => ((typeof result) === 'number') && (result % 5 === 0)).map(result => Math.pow(2, result)));
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((accumulator, currentValue) => Number(accumulator.height) < Number(currentValue.height) ? accumulator : currentValue).name;
};
