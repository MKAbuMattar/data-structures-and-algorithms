...
const validateEmail = (email) => {
  // Solution code here...
  const pattern = /^\w+\.?\w+@\w+.(com|net|org)$/;
...
const findTagNames = elements => {
  ...
  // return elements.map(string => string.match(pattern)).reduce((arr, result) => arr.concat(result.map(result => result.slice(1, (result.length - 1)))), []);
  // too long 🤦‍♂️

  // const pattern = /\/\w*/g;
  // let arr = [];
  // return arr.concat(...elements.map(result => result.match(pattern)));
  // hmmmm 🤔, no

  // const pattern = /\/\w*/g;
  // return [].concat(...elements.map(result => result.match(pattern)));
  // hmmmm 🤔, no

  return [].concat(...elements.map(result => result.match(/\/\w*/g)));
  // this one, yes this one 👨‍💻
};

// all thanks to https://regexr.com/ & https://regex101.com/ for helping me to solve this challenge 7
