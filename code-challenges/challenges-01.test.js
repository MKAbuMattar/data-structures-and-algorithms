...
const allUpperCase = (arr) => {
  ...
  let newArr = [];
  arr.forEach(result => {
    newArr.push(result.toUpperCase());
  });
  return newArr;
};
...
const speaker = (words, callback) => {
  // Solution code here...
  let newArr = [];
  words.forEach((result, index) => {
    newArr.push(callback(words[index]));
  });
  return newArr;
...
const createList = (availableItems) => {
  // Solution code here...
  let newArr = [];
  availableItems.forEach((result, place) => {
    if (availableItems[place].available === true) {
      newArr.push(availableItems[place].name);
    }
